...
const Home = ({ setPage, setSelectedFriend }) => {
    ...
    return (
        <div className="bg-gray-50 min-h-screen">
            <header className="text-center pt-16 pb-12 px-4">
                ...
                <button className="btn bg-[#2D4A41] hover:bg-[#1f352e] text-white border-none px-8 font-semibold shadow-md">
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                        {friends.map((friend) => (
                            <div 
                                key={friend.id}
                                /* Requirement 4.5: Navigate to Detail Page on click */
                                onClick={() => {
                                    setSelectedFriend(friend);
                                    setPage('details');
                                }}
                                className="group bg-white rounded-3xl p-8 shadow-sm border border-transparent hover:border-[#2D4A41]/20 hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col items-center text-center relative overflow-hidden"
                            >
                                <div className="relative mb-4">
                                    <img 
                                        src={friend.picture} 
                                        alt={friend.name} 
                                        className="w-24 h-24 rounded-full object-cover ring-4 ring-gray-50 group-hover:ring-[#2D4A41]/10 transition-all" 
                                    />
                                </div>
                                
                                <h3 className="text-xl font-bold text-gray-800 group-hover:text-[#2D4A41] transition-colors">
                                    {friend.name}
                                </h3>
                                ...
                                <div className="flex flex-wrap justify-center gap-2 mb-8">
                                    {friend.tags.map(tag => (
                                        <span key={tag} className="px-3 py-1 bg-gray-100 text-gray-600 text-[10px] font-bold rounded-full uppercase tracking-wider">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
